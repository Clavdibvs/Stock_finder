...
| D27 | **Classificatore eventi news deterministico** (parole chiave, senza AI) | chiude il buco principale del funzionamento senza AI: rileva M&A/FDA/offering dai titoli e alza il gate binario di cautela; errore verso la prudenza (direzione sicura), richiede ≥2 origini per gli eventi binari, mai converte rumor in fatto, tutto tracciabile (keyword+origini). Più adatto di un LLM per un tool di rischio auditabile; l'AI resta upgrade opzionale per la lettura fine del contenuto |
